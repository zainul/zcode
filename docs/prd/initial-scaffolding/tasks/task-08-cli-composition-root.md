...
## Objective
Build `crates/cli` (`zcode`) with `clap v4`, a `version` subcommand printing `zcode v<version> (git: <sha>, profile: <profile>)`, build-metadata via `vergen-gix`, a `#[tokio::main(flavor="current_thread")]` entry, and the **composition root** `wire()` that constructs concrete infra-backed `App`s (stubbed use-cases) and fails fast with a typed error on unresolved ports.
...
3. Create `crates/cli/src/main.rs` — `#![forbid(unsafe_code)]`, `#[tokio::main(flavor="current_thread")]`, calls `zcode::cli::run()`, maps error to `eprintln!("zcode: {e}")` + `ExitCode::from(1)`.
4. Create `crates/cli/src/lib.rs` exposing `pub mod cli;`.
5. Create `crates/cli/src/cli/mod.rs`:
   - `Cli` derive (`#[command(name="zcode", version, about)]`) + `Commands` enum (`Version`).
...
## Test-case scenario
- `cargo run --quiet -- version` prints exactly `zcode v0.1.0 (git: <sha>, profile: <profile>)`; an unknown subcommand yields a clap error (exit 2), not a panic.
...
size -A target/release/zcode             # L2 < 8MB proxy
```
**Pass criteria:** version output matches the format; `cargo tree -p zcode` lists all layers; no `unsafe` in `cli` (NFR-PORT-02).
