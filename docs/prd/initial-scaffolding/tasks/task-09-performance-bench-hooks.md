...
1. Create `benches/Cargo.toml` — `ag-benches` crate, dep on `domain` + `criterion`; `[[bench]] name="smoke" harness=false`; `[lib] bench=false`.
...
3. Add a `make size` target printing `target/release/ag` bytes (task-11).
...
ls -la target/release/ag               # L2 < 8MB
```
