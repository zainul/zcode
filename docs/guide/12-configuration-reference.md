...
NAME/<model>` selects one *and* the model to use on it — both beat the file and
the environment. That flag is the only place the `<provider>/<model>` pair is
written: in a file the two are separate keys.
